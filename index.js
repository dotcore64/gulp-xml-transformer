...
const update = (file) => (xml) =>
  Object.assign(file, {
    contents: file.isBuffer() ? Buffer.from(xml) : through().end(xml),
  });

const getTransformStream = (transformer) =>
  through.obj(
    callbackify(
      clone((file, enc) =>
        file.isNull()
  ...
  switch (typeof transformations) {
    ...
    case "object": {
      return getTransformStream(
        objectTransformer(arrify(transformations), nsUri),
      );
    }
    case "undefined": {
    ...
    default: {
      throw new PluginError(
        PLUGIN_NAME,
        "transformations option must be a function or an object",
      );
    }
  }
};
