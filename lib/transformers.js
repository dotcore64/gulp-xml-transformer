...
export const func = (tranformation) => (xml) =>
  Promise.resolve(tranformation(xml, libxmljs)).then((newDoc) =>
    newDoc.toString(),
  );
...
export const obj = (transformations, nsUri) => (doc) => {
  transformations.forEach((transformation) => {
    ...
    const { path, text, attrs, attr, isMandatory = true } = transformation;

    if (!(elem instanceof libxmljs.XMLElement)) {
      if (isMandatory) {
    ...
    arrify(attrs ?? attr).forEach((attr) => {
      Object.keys(attr).forEach((key) => {
        const oldAttr = elem.getAttribute(key);
